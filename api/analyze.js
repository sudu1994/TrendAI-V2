...
async function fetchRakutenData(keyword) {
  ...
  if (!APP_ID) {
    console.error('[Rakuten] RAKUTEN_APP_ID not configured');
    return {
      source: 'live', status: 'empty', error: 'RAKUTEN_APP_ID missing',
  ...
  const attemptSearch = async (searchKeyword) => {
    ...
    return axios.get('https://app.rakuten.co.jp/services/api/IchibaItem/Search/20220601', {
      params: { applicationId: APP_ID, format: 'json', keyword: searchKeyword, hits: 10, sort: '-reviewCount' },
      timeout: 6000,
    });
  };
...
async function fetchYoutubeData(keyword) {
  ...
  if (!YT_KEY) {
    ...
    return {
      source: 'live', status: 'empty', error: 'YOUTUBE_API_KEY missing',
      message: 'API key not configured',
  ...
  const attemptSearch = async ({ withRegion = true } = {}) => {
    const params = {
      ...
      maxResults: 8,        // FIX: was 8 but publishedAfter killed results
      key: YT_KEY,
    };
    // publishedAfter REMOVED from primary — was 6 months, too restrictive
    if (withRegion) {
